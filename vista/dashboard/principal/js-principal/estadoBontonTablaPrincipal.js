function activar(btn) {
    ...
    cambiarEstado(btn, 'Seleccionado');
}

function desactivar(btn) {
    console.log('Botón de desactivación presionado');
    cambiarEstado(btn, 'No seleccionado');
}

function cambiarEstado(btn, nuevoEstado) {
    console.log('Cambiando estado a:', nuevoEstado);

    var table = $('#myTable').DataTable(); // Obtén la instancia de DataTables
        ...
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            if (nuevoEstado === 'No seleccionado') {
                ...
                Swal.fire({
                    position: "center",
                    icon: "success",
                    title: "Postulante no seleccionado",
